...
bool pickRay::chkPick(LPD3DXMESH mesh, info* out_info)
{
	// 클릭 좌표 확인
	...
	D3DXVECTOR3 pickPos;
	D3DXMATRIXA16* mProjection = GET_CAMERA()->getMatrixProjectionPoint();
	...
	// 뷰, 월드 역행렬
	D3DXMATRIXA16 mWorld = GET_CAMERA()->getMatrixFinal() * (*GET_CAMERA()->getMatrixViewPoint());
	D3DXMatrixInverse(&mWorld, NULL, GET_CAMERA()->getMatrixViewPoint());

	// 클릭 좌표 2D -> 3D 변환
	D3DXVECTOR3 & rayDir = out_info->rayDir;
	D3DXVECTOR3 & rayOrigin = out_info->rayOrigin;
